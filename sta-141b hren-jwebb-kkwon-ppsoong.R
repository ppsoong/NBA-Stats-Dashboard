...
server <- function(input, output, session) {
  ...
  output$plot3 <- renderPlotly({
    plot_ly(data = player_team(), x = ~X, y = ~Y, color = ~Player) %>% # added layout jw
      ...
      layout(
        paper_bgcolor = "#2a3439", plot_bgcolor = "#2a3439",
  ...
  output$their_stats <- renderPlot({
    par(bg = "#2a3439")
    pie(as.numeric(picked_player()[1, ]), colnames(picked_player()),
      col.lab = "white", col.axis = "#ffffff", col = c("#74ee15", "#ffe700", "#f000ff", "#001eff", "#4deeea"),
      main = "The 5 Stat Breakdown: Rebounds, Points, Assists Steals, Blocks \n Average per Game", col.main = "white"
    ) # , "#FF8B8B"))
  })
